...
const query = qs.stringify({
  ...
  const data = await fetchAPI(`/asset?${query}`)
...
export const getOrganData = async (slug) => {
  const organQuery = qs.stringify({
    ...
    populate: {
      area_organs: { populate: ["Images", "Specs", "PrimaryImage"], sort: ["id:asc"] },
      Address: "*",
    },
  });
...
export const getOrganTeachers = async () => {
  const teachersData = await fetchAPI(
    `/organ-teachers?sort=lastName&populate=*`
  );
...
export const getSubOrganists = async () => {
  const subOrganistsData = await fetchAPI(
    `/sub-organists?sort=lastName&populate=*`
  );
